'''
Corpus 2.
Code for turing raw input into the right format.
'''
...
def check_length(sequence):
    ...
    if length < limit['minq'] or length > limit['maxq']:
        return None
    return sequence
...
# gets word to index for a file so it doesn't have to be recalculated
def get_w2idx():

    lines = read_lines(filename='datasets/facebook2/qa2.txt')
...
# get arrays for a given input sentence, used to find the model output
def process_input(l, w2idx):

    line = l.lower()
    line = filter_line(line, EN_WHITELIST)
    if check_length(line) is None:
        return None
    tokens = line.split(' ')
    tokenarr = [tokens]

    aline = "This is a set response."
    aline = aline.lower()
    aline = filter_line(aline, EN_WHITELIST)
    atokens = aline.split(' ')
    atokenarr = [atokens]

    idx_q, idx_a = zero_pad(tokenarr, atokenarr, w2idx)
    return idx_q, idx_a

def process_data():

    print('\n>> Read lines from file')
    lines = read_lines(filename='qa2.txt')
